...
type BusinessType = {
  id: number; userId: string; businessName: string; businessSlug: string | null
  logoUrl: string | null; brandColor: string | null; smsProvider: string | null
  requireEmail: boolean | null; smsSenderName: string | null
  maxAdvanceBooking: number | null; durationOptions: string | null
  createdAt: Date; updatedAt: Date
} | null

type AppointmentType = {
  id: number; userId: string; customerName: string; customerEmail: string
  eventStart: Date; duration: number; status: string
  _businessName?: string
  [key: string]: unknown
}

type ContactType = { id: number; name: string; email: string | null; phone: string | null }
type TaskType = { id: number; title: string; status: string; dueDate: Date | null }
type OppType = { id: number; title: string; stage: string | null; value: number | null; probability: number | null }
type TimelineType = { id: number; title: string; createdAt: Date }
type InvoiceType = { total: number | null; status: string | null }
type HealthScoreType = { score: number; factors: { label: string; score: number; impact: string }[] }

interface DashboardContentProps {
  userName: string
  configName: string
  terms: {
    appointmentPlural: string; customerPlural: string; taskPlural: string
    customer: string; task: string; deal: string; dealPlural: string; invoice: string
    appointmentPluralLower: string
  }
  bookingLink: string | null
  businessSlug: string | null
  insights: AIInsight[]
  todayAppts: AppointmentType[]
...
export default function DashboardContent(props: DashboardContentProps) {
  const {
    userName, configName, terms, bookingLink, businessSlug, insights,
    todayAppts, upcoming, allContacts, openTasks, pipelineValue,
    outstandingAmount, openOpps, recentTimeline, healthScore, calendar
  } = props
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Here&apos;s what&apos;s happening with your {configName.toLowerCase()} business today
          </p>
        </div>
        {bookingLink && (
          <Link
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard icon={<CalendarCheck className="w-4 h-4" />} label={`Today's ${terms.appointmentPlural}`} value={todayAppts.length} color="text-blue-500" />
        <StatCard icon={<Clock className="w-4 h-4" />} label={`Upcoming ${terms.appointmentPlural}`} value={upcoming.length} color="text-violet-500" />
        <StatCard icon={<Users className="w-4 h-4" />} label={terms.customerPlural} value={allContacts.length} color="text-emerald-500" />
        <StatCard icon={<CheckSquare className="w-4 h-4" />} label={`Open ${terms.taskPlural}`} value={openTasks.length} color="text-amber-500" />
        <StatCard icon={<TrendingUp className="w-4 h-4" />} label="Pipeline Value" value={`$${(pipelineValue / 100).toFixed(0)}`} color="text-cyan-500" />
        <StatCard icon={<DollarSign className="w-4 h-4" />} label="Outstanding" value={`$${(outstandingAmount / 100).toFixed(0)}`} color={outstandingAmount > 0 ? 'text-red-500' : 'text-emerald-500'} />
      </div>
            ...
            {todayAppts.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {todayAppts.slice(0, 5).map(apt => (
                  <div key={apt.id as number} className="rounded-md border border-border bg-card p-3 flex items-center justify-between gap-3 hover:bg-muted/50 transition-colors">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-medium text-foreground">
                        {new Date(apt.eventStart).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                      </p>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {upcoming.slice(0, 5).map(apt => {
                  const isClientAppt = '_businessName' in apt && apt._businessName
                  return (
                    <div key={apt.id as number} className="rounded-md border border-border bg-card p-3 flex items-center justify-between gap-3 hover:bg-muted/50 transition-colors">
                      <div className="min-w-0 flex-1">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-sm font-medium text-foreground">
                          {new Date(apt.eventStart).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {new Date(apt.eventStart).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })} ({apt.duration}min)
                        </p>
                      </div>
                    </div>
                  )
